...
class LifeLogDatabaseTest {
    ...
    @Test
    fun insertLifeLog_addsEntityCorrectly() =
        runTest {
            val newLog =
                LifeLogEntity(
                    date = "2025-10-06",
    ...
    @Test
    fun updateLifeLog_updatesExistingEntity() =
        runTest {
            val logs = dao.getLifeLogs().first()
    ...
    @Test
    fun upsertLifeLog_insertNewEntity() =
        runTest {
            val newLog =
                LifeLogEntity(
                    id = "test-uuid-6",
    ...
    @Test
    fun deleteLifeLogById_removesEntity() =
        runTest {
            val logs = dao.getLifeLogs().first()
            val target = logs.first()
